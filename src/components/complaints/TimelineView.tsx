      ...
      <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
        ...
        {createdAt && (
          <div className="relative group">
            <div className="absolute -left-6 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-slate-900 ring-4 ring-slate-900 border border-amber-500/60">
              <Clock className="w-3 h-3 text-amber-400" />
            </div>
            <div className="text-xs">
              <div className="flex items-center justify-between">
                <span className="font-bold text-white">Grievance Registered</span>
                <span className="text-[11px] font-mono text-slate-500">
                  {formatDate(createdAt)}
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                Citizen <span className="text-slate-300 font-medium">{initialReporter}</span>{' '}
        ...
        {updates.map((update, index) => {
          ...
          return (
            <div key={update.id || index} className="relative group">
              <div
                className={`absolute -left-6 top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-slate-900 ring-4 ring-slate-900 border ${
                  isResolved ? 'border-emerald-500' : 'border-slate-700'
                }`}
              >
              ...
              <div className="text-xs">
                ...
                {(update.notes || update.comment) && (
                  <p className="text-xs text-slate-300 mt-1 bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                    {update.notes || update.comment}
                  </p>
                )}
                ...
                <div className="mt-1 text-[10px] text-slate-500 flex items-center gap-1">
                  <span>Logged by:</span>
                  <span className="text-slate-400 font-medium">{update.updatedBy}</span>
                </div>
              </div>
            </div>
          );
