...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/75 backdrop-blur-sm animate-fade-in">
      <div
        className={`relative w-full ${maxWidthClass} rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors`}
      >
        {/* Header */}
        ...
        {/* Body */}
        <div className="p-6 overflow-y-auto custom-scrollbar text-slate-800 dark:text-slate-200">{children}</div>
      </div>
    </div>
  );
};
